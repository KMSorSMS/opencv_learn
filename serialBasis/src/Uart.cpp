...
void Uart::InitSerialPort(std::string dev)
{
    ...
    int databits = 8;
    int stopbits = 1;
    SetUp(speed, flow_ctrl, databits, stopbits);//设置串口
}
...
{
    const char* _dev = new char[32];//串口号
    _dev = dev.c_str();//将string转化为const char*
    ...
    ioctl(fd,TIOCMBIS,&DTR_flag);//Set RTS pin,即设置RTS为高电平，发送数据
    return ;
}
...
void Uart::SetUp(int speed, int flow_ctrl, int databits, int stopbits)
{
    ...
    */
    if(tcgetattr(fd, &options)){ //获取串口的属性，并保存到options中
